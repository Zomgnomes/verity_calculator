...
function processSelections(): void {
  ...
  console.log(
    `middle_has: ${middle_has} middle_goal: ${middle_goal} middle_wrong_parts: ${middle_wrong_parts}`
  )
  console.log(
    `right_has: ${right_has} right_goal: ${right_goal} right_wrong_parts: ${right_wrong_parts}`
  )
  ...
  if (
    ...
    console.log(
      `left_has: ${left_has} left_goal: ${left_goal} left_wrong_parts: ${left_wrong_parts}`
    )
    console.log(
      `middle_has: ${middle_has} middle_goal: ${middle_goal} middle_wrong_parts: ${middle_wrong_parts}`
    )
    ...
    if (loc_1 !== 'L' && left_goal.includes(shape_1) && !left_has.includes(shape_1)) {
      shape_2 = left_wrong_parts.slice(0, 1)
      loc_2 = 'L'
      left_has = left_has.replace(shape_2, shape_1).split('').sort().join('')
    } else if (loc_1 !== 'M' && middle_goal.includes(shape_1) && !middle_has.includes(shape_1)) {
  ...
  if (
    ...
    if (loc_3 === 'L') {
      if (
        ...
        middle_has = middle_has
          .replace(shape_4, shape_3 as string)
          .split('')
          .sort()
          .join('')
      } else {
    ...
    if (loc_3 === 'L') {
      left_has = left_has
        .replace(shape_3 as string, shape_4)
        .split('')
        .sort()
        .join('')
    } else if (loc_3 === 'M') {
...
<template>
  <div
    class="text-gray-600 bg-gray-200 dark:bg-gray-800 dark:text-gray-300 rounded-b-lg flex flex-col grow justify-around min-h-fit max-h-5/6 min-w-fit"
  >
    <div>
      ...
      <div class="flex justify-around flex-nowrap">
        <div>
          ...
          <Shape2dOptions :selections="leftInside" @shapeSelected="updateInside(0, $event)" />
        </div>
        <div>
          <h3 class="text-center text-lg">Middle</h3>
          <Shape2dOptions :selections="middleInside" @shapeSelected="updateInside(1, $event)" />
        </div>
        <div>
          <h3 class="text-center text-lg">Right</h3>
          <Shape2dOptions :selections="rightInside" @shapeSelected="updateInside(2, $event)" />
        </div>
      </div>
    </div>
    <div>
      ...
      <div class="flex justify-around flex-nowrap">
        <div>
          <h3 class="text-center text-lg">Left</h3>
          <Shape3dOptions :selections="leftOutside" @shapeSelected="updateOutside(0, $event)" />
        </div>
        <div>
          <h3 class="text-center text-lg">Middle</h3>
          <Shape3dOptions :selections="middleOutside" @shapeSelected="updateOutside(1, $event)" />
        </div>
        <div>
          <h3 class="text-center text-lg">Right</h3>
          <Shape3dOptions :selections="rightOutside" @shapeSelected="updateOutside(2, $event)" />
        </div>
      </div>
    </div>
    <div>
      ...
      <ol class="flex flex-col justify-around align-middle flex-nowrap">
        <li class="p-2 mx-auto">
          <input
            v-model="wave1"
            type="text"
            class="h-[40.5px] w-[50vw] text-black pl-2 font-bold"
          />
        ...
        <li class="p-2 mx-auto">
          <input
            v-model="wave2"
            type="text"
            class="h-[40.5px] w-[50vw] text-black pl-2 font-bold disabled"
          />
